...
class ModBot(discord.Client):
    ...
    async def on_reaction_add(self, reaction, user):
        ...
        print(f"on_reaction_add fired: reaction={reaction.emoji}, user={user}, channel={reaction.message.channel}")
    ...
    async def handle_mod_channel_message(self, message):
        ...
        try:
            ...
            # Simulate the action
            if action == "warn":
                await message.channel.send(f"Warning sent to user {report.report_data['reported_message'].author.name}")
            elif action == "delete":
                await message.channel.send(f"Message from {report.report_data['reported_message'].author.name} would be deleted")
            elif action == "ban":
                await message.channel.send(f"User {report.report_data['reported_message'].author.name} would be banned")
