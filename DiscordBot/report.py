...
class Report:
    ...
    async def handle_message(self, message):
        ...
        if self.state == State.AWAITING_MESSAGE:
            ...
            bot_message = await message.channel.send("Please select post type:\nReact with:\n1️⃣ for Fraud\n2️⃣ for Other")
        ...
        if self.state == State.AWAITING_POST_TYPE:
            if message.content == "1️⃣":  # Fraud
                ...
                bot_message = await message.channel.send("Please select fraud type:\nReact with:\n1️⃣ for Phishing\n2️⃣ for Investment Scam\n3️⃣ for E-Commerce\n4️⃣ for Account Takeover")
                self.last_bot_message = bot_message
                await bot_message.add_reaction("1️⃣")
                await bot_message.add_reaction("2️⃣")
                ...
                bot_message = await message.channel.send("Thanks for reporting to fraud bot. Do you believe this message is an attempt at fraud?\nReact with:\n1️⃣ for Yes\n2️⃣ for No")
                self.last_bot_message = bot_message
                await bot_message.add_reaction("1️⃣")
                await bot_message.add_reaction("2️⃣")
                return []
        ...
        if self.state == State.AWAITING_FRAUD_TYPE:
            if message.content == "1️⃣":  # Phishing
                ...
                bot_message = await message.channel.send("Please select scam type:\nReact with:\n1️⃣ for Crypto\n2️⃣ for Selling Counterfeit Items\n3️⃣ for Other")
                self.last_bot_message = bot_message
                await bot_message.add_reaction("1️⃣")
                await bot_message.add_reaction("2️⃣")
                await bot_message.add_reaction("3️⃣")
                ...
                bot_message = await message.channel.send("Please select e-commerce type:\nReact with:\n1️⃣ for Fake Online Store\n2️⃣ for Selling Counterfeit Items")
                self.last_bot_message = bot_message
                await bot_message.add_reaction("1️⃣")
                await bot_message.add_reaction("2️⃣")
                return []
                ...
                bot_message = await message.channel.send("Please select takeover type:\nReact with:\n1️⃣ for Unauthorized Login\n2️⃣ for Unauthorized Post/Message from my account")
                self.last_bot_message = bot_message
                await bot_message.add_reaction("1️⃣")
                await bot_message.add_reaction("2️⃣")
                return []
        
        if self.state in [State.AWAITING_PHISHING_TYPE, State.AWAITING_INVESTMENT_SCAM_TYPE, State.AWAITING_ECOMMERCE_TYPE]:
    ...
    async def show_block_decision(self, channel):
        bot_message = await channel.send("Thank you for reporting. Our content moderation team will review the account and/or post. Would you like to block the user?\nReact with:\n1️⃣ for Yes\n2️⃣ for No")
        self.last_bot_message = bot_message
        await bot_message.add_reaction("1️⃣")
        await bot_message.add_reaction("2️⃣")
        return []
