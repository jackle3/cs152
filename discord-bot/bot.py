...
class ModBot(commands.Bot):
    ...
    async def setup_hook(self):
        ...
        # Add the list reports command
        @self.tree.command(name="list_reports", description="List all active reports (Moderators only)")
        async def list_reports(interaction: discord.Interaction):
            """List all active reports in the mod channel"""
            # Check if this is being used in a mod channel
            if not self._is_mod_channel(interaction.channel):
                await interaction.response.send_message(
                    "❌ This command can only be used in moderator channels.", 
                    ephemeral=True
                )
                return
            ...
            guild_reports = []
            for report_id, (user_id, report) in self.active_reports.items():
                if report.reported_message.guild.id == interaction.guild.id and report.active:
            ...
            for i, report in enumerate(guild_reports[:10]):  # Limit to 10 reports to avoid embed limits
                ...
                if report.abuse_category and report.abuse_category in self._get_abuse_types():
                    abuse_type = self._get_abuse_types()[report.abuse_category]
                    type_text = f"{abuse_type.emoji} {abuse_type.label}"
                else:
                    type_text = "❓ Other"

                # Get subtype if available
                if report.subtypes and report.abuse_category in self._get_abuse_types():
                    abuse_type = self._get_abuse_types()[report.abuse_category]
                ...
                embed.add_field(
                    ...
                    value=f"**Type:** {type_text}\n"
                          f"**Reporter:** <@{self.active_reports[report.id][0]}>\n"
                          f"**Target:** {report.reported_message.author.mention}\n"
                          f"**Channel:** <#{report.reported_message.channel.id}>",
                    inline=True
                )
